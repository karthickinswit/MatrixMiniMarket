...
function createNorm(tx, success, error) {
    var createStatement = "CREATE TABLE IF NOT EXISTS mxpg_norm(norm_id TEXT, norm_name TEXT, option_id TEXT, remark_id TEXT)";
...
function selectNorms(db, channelId, productId, priority, isFrontage, fn) {
    ...
    var query = "select t1.norm_id, t1.store_score, t1.norm_order, t2.norm_name, t3.option_id, t3.option_name, t4.remark_id, t4.remark_name from mxpg_pn_map t1 JOIN mxpg_norm t2 JOIN mxpg_option t3 JOIN mxpg_remark t4 where t1.product_id=" + productId + " and t1.channel_id=" + channelId + " and t1.norm_id=t2.norm_id and t2.option_id=t3.option_id and t2.remark_id=t4.remark_id order by t1.norm_order ASC";
    db.transaction(function(tx){
