...
function populateAllStoreTable(db, storesDetails, callback, error) {
    ...
    db.transaction(function(tx){
        for(var i = 0; i < length; i++){
            ...
            tx.executeSql('INSERT OR replace INTO mxpg_store(audit_id, store_id, id, store_code, store_name, mkt_id, mkt_name, chl_id, chl_name, dbtr_id, dbtr_name, loc_id, loc_name, brch_id, brch_name, adtr_id, adtr_name, adtr_code, addr, due, lat, lng, is_fresh, store_visit, last_visit_date) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);',
                [auditId, store.sId, store.id, store.sCodeByMx, store.sName, store.mktId, store.mktName, store.chanId, store.chanName, store.distId, store.distName, store.locId, store.locName, store.mxBrchId, store.mxBrchName, empId, empName, empCode, store.sAdds, due, latitude, longitude, store.isFreshAudit, store.storeVisit, store.lastAuditedDate || "NA" ], function(tx, results){
