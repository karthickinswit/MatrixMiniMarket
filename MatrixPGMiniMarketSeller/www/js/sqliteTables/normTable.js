...
function createNorm(tx, success, error) {
    var createStatement = "CREATE TABLE IF NOT EXISTS mxpg_norm(norm_id TEXT, norm_name TEXT, option_id TEXT, remark_id TEXT, get_reason TEXT, norm_type TEXT)";
...
function populateNormTable(db, norms, success, error) {
    db.transaction(function(tx){
        for(var i = 0; i < norms.length; i++){
            var norm = norms[i];
            tx.executeSql('INSERT OR replace INTO mxpg_norm(norm_id, norm_name, option_id, remark_id, get_reason, norm_type) VALUES (?,?,?,?,?,?);',
                [norm.normId, norm.normName, norm.optionId, norm.remarkId, norm.reason, norm.normType]
            , success, error);
        }
    });
}
...
function selectNorms(db, channelId, productId, priority, isFrontage, fn) {
    ...
    var query = "select t1.norm_id, t1.store_score, t1.norm_order, t1.product_id, t2.norm_name, t2.get_reason, t2.norm_type, t3.option_id, t3.option_name, t4.remark_id, t4.remark_name from mxpg_pn_map t1 JOIN mxpg_norm t2 JOIN mxpg_option t3 JOIN mxpg_remark t4 where t1.product_id=" + productId + " and t1.channel_id=" + channelId + " and t1.norm_id=t2.norm_id and t2.option_id=t3.option_id and t2.remark_id=t4.remark_id order by t1.norm_order ASC";
    db.transaction(function(tx){
        tx.executeSql(query , [], function(tx, response) {
            ...
            for(var i = 0; i < len; i++){
                ...
                temp.show1 = "block";
                temp.show2 = "none";
                temp.showRemark = "none";
                temp.options = [];
                temp.yes = [];
                temp.no = [];
                temp.reason = (row.get_reason == 1)? true:false;
                 ...
                 if(row.norm_type == inswit.FIELD_TYPES.INT) {
                    temp.textBox = true;
                    temp.typeINT = true;
                    temp.options.push({"optionName":row.givenValue, "optionId":row.option_id});
                }else if(row.norm_type == inswit.FIELD_TYPES.TEXT) {
                    temp.textBox = true;
                    temp.typeTEXT = true;
                    temp.options.push({"optionName":row.givenValue, "optionId":row.option_id});
                }else if(row.norm_type == inswit.FIELD_TYPES.OPTION) {
