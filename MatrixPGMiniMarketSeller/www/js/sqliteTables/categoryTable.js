...
function selectBrands (db, auditMonthId, channelId, category_id, fn) {
    var query = "select t1.category_id,t1.product_id, t2.product_name, t2.channel_name,t2.is_hotspot, t2.priority, t2.is_frontage, t2.brand_order, t2.channel_id  from mxpg_pn_map t1 join mxpg_product t2 where t1.category_id = "+ category_id +" and t2.product_id = t1.product_id and t1.channel_id = "+ channelId +" and t1.audit_month_id ="+ auditMonthId +" group by t1.product_id order by t1.category_id asc";
    ...
    db.transaction(function(tx){
        tx.executeSql(query, [], function(tx, response){
            ...
            tx.executeSql(query, [], function(tx, response){
                ...
                var brandCount;
                if(len > 0){
                    brandCount = response.rows.item(0).brandCount;
                }
                if(completedBrandCount >= brandCount){
                    success(true)
                }
                console.log("results"+ completedBrandCount + brandCount);
            });
        }, error);
    });
