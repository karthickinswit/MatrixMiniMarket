...
function createCompProductTable(tx, success, error) {
    var createStatement = "CREATE TABLE IF NOT EXISTS mxpg_comp_products(store_id TEXT, store_name TEXT, product_id TEXT, product_name TEXT, norm_id TEXT, norm_name TEXT, option_id TEXT, option_name TEXT, remark_id TEXT, remark_name TEXT, image TEXT, image_uri TEXT, audit_id TEXT, store_score BOOLEAN, priority NUMBER, image_id)";
    tx.executeSql(createStatement, [], success, error);
...
function populateCompProductTable(db, product, callback) {
    ...
    db.transaction(function(tx){
        for(var i = 0; i < length; i++){
            ...
            tx.executeSql('INSERT OR replace INTO  mxpg_comp_products(store_id, store_name, product_id, product_name, norm_id, norm_name, option_id, option_name, remark_id, remark_name, image, image_uri, audit_id, store_score, priority, image_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);',
                [storeId, storeName, norm.productId, norm.productName, norm.normId, norm.normName, norm.optionId, norm.optionName, norm.remarkId, norm.remarkName, image, imageURI, auditId, norm.isConsider, priority, imageId]);
...
function selectProductsToVerify(db, auditId, storeId, productId, fn) {
    ...
    var query = "select store_id as storeId, product_id as productId, product_name as productName, norm_id as normId, norm_name  as normName, option_id  as optionId, option_name as optionName, remark_id as remarkId, remark_name as remarkName, image, image_uri as imageURI, audit_id as auditId, store_score as isConsider, priority from mxpg_comp_products where audit_id='" + auditId + "'";
